...
class ActionController extends Controller {
    ...
    public function do_login(){
        if(IS_POST){
            ...
            $result = $user_model->where($cdt)->find();
            if(!empty($result)){
                //对密码混淆加密
                $salt = $result['salt'];
                ...
                if($result['passwd'] == $passwd){
                    ...
                    $user_model->where($cdt)->save($data);
    ...
    public function do_register(){
        ...
        if(!empty($userName) && !empty($passwd) && !empty($email)){
          ...
          //判断用户是否存在
          $cdt['userName'] = $userName;
          if(!empty($user_model->where($cdt)->find()))$this->error('用户已存在，请直接登录！');
          else{
            //判断邮箱格式
            ...
            if(!empty($user_model->where($cdt)->find()))$this->error('该邮箱已注册过，请直接登录！');
            //对密码混淆加密
            ...
            if($user_model->data($data)->add())$this->success('注册成功！',U('Action/login'));
    ...
    public function deal_modify(){
        ...
        switch ($type) {
            case 'piece':
                A('Piece')->modify($id,$tag,$content,$visible);
                break;
            case 'essay':
                A('Essay')->modify($id,$title,$tag,$content,$visible);
                break;
            case 'diary':
                A('Diary')->modify($id,$title,$tag,$content,$visible);
                break;
            default:
                $this->error('请求失败！');
                break;
