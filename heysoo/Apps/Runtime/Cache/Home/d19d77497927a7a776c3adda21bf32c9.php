...
<form action="<?php echo U('Action/do_register');?>" class="register_form" method="post">
...
<?php if($register_invite_code_on == true): ?><div class="input-group">
<span class="input-group-addon">邀请码</i></span>
<input class="inviteCode form-control" name="inviteCode"  type="text" placeholder="邀请码" title="向已注册用户索取邀请码">
</div><?php endif; ?>

<?php if($register_verify_code_on == true): ?><div class="input-group">
...
<button type="submit" class="btn btn-primary btn-block register">注  册</button>
<button type="button" class="btn btn-primary btn-block login" onclick="window.location.href='<?php echo U('Action/login');?>'">登  录</button>
</form>
</div>
</div>
