 ...
 * Think 系统函数库
 */
function set_history_url($url){
    if(isset($_COOKIE['history_url'])){
        $h_url = $_COOKIE['history_url'];
        array_push($h_url,$url);
        $_COOKIE['history_url'] = $h_url;
    }else{
        $h_url = array();
        array_push($h_url,$url);
        $_COOKIE['history_url'] = $h_url;
...
function check_history_url(){
    if(isset($_COOKIE['history_url'])){
        if(!empty($_COOKIE['history_url'])){
            $h_url = $_COOKIE['history_url'];
            $url = array_pop($h_url);
            $_COOKIE['history_url'] = $h_url;
            redirect($url);
...
//将下划线转为驼峰命名
function convertUnderlineToCamel($str,$ucfirst = true){
